...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(100, 10, utf8_decode("REPORTE DE PRODUCCIONES ACADÉMICAS"), 0, 1, 'C', 0);
      $this->Ln(7);
      ...
      /*la suma del ancho de la tabla
         debe de dar 275*/
      $this->SetFillColor(0, 100, 280); //colorFondo
      $this->SetTextColor(255, 255, 255); //colorTextodelafranja
      $this->SetDrawColor(163, 163, 163); //colorBorde
      $this->SetFont('Arial', 'B', 11);
      $this->Cell(10, 10, utf8_decode('ID'), 1, 0, 'C', 1);
      $this->Cell(72, 10, utf8_decode('NOMBRE'), 1, 0, 'C', 1);
      $this->Cell(50, 10, utf8_decode('FECHA DE PUBLICACIÓN'), 1, 0, 'C', 1);
      $this->Cell(110, 10, utf8_decode('PROYECTO DE INVESTIGACIÓN DEL CUAL SE GENERÓ'), 1, 0, 'C', 1);
...
$consulta = "SELECT id_producciones_academicas, nombre, fecha_publicacion,
            proyec_inv_gen, tipo FROM producciones_academicas";
...
while ($row = mysqli_fetch_array($result)) {
   ...
   /* TABLA */
   $id = utf8_decode($row['id_producciones_academicas']);
    $nombre = utf8_decode($row['nombre']);
    $proyecto = utf8_decode($row['proyec_inv_gen']);
    $tipo = utf8_decode($row['tipo']);

    $pdf->Row([$id, $nombre, $fecha_formateada, $proyecto, $tipo]);
}
...
$pdf->Output('Reporte Producciones Academicas.pdf', 'I');//nombreDescarga, Visor(I->visualizar - D->descargar)
